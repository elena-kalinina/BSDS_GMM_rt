function [Priors, Mu, Sigma, Pix_tot, gmmcluster] = EM_update_directMethod_upd(Data, Priors0, Mu0, Sigma0, Pix0)
%
...
while 1
  ...
  for j=1:nbData
    ...
    [argvalue, argmax]=max(Pix(j, :));
    cluster=argmax;
   % fprintf('Volume %d belongs to cluster %d \n', j, cluster);
  end
  %Compute cumulated posterior probability
  E = sum(Pix,1);
  %% M-step %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
