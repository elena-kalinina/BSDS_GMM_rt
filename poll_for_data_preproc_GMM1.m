function poll_for_data_preproc_GMM1(SubjectID, sessionN, cfg)
...
%         end
        ...
        myvol2=datavol(maskvol2>0);
          %  myvol=(myvol-mean(myvol))/std(myvol);
            ...
            if SerPor.BytesAvailable
                ...
                vol_counter=1;
                if signal
                    tc(end+1)=((mean(myvol1)-mean(myvol2))/mean(myvol2))*100;
       ...
       % onset(end+1)=str2double(fscanf(SerPor, '%s'));
          ...
          if numProper == cfg.breakPoint
              ...
              signal
          if (numProper > cfg.breakPoint) && (signal>0)
              
              %   if SerPor.BytesAvailable
              
              
              [MM(counter).Priors, MM(counter).Mu, MM(counter).Sigma, MM(counter).Pix, gmmcluster] = ...
                  EM_update_directMethod_upd(tc(end), MM(counter-1).Priors, MM(counter-1).Mu, MM(counter-1).Sigma, MM(counter-1).Pix);
              ...
              if gmmcluster>clustering(end-1) || vol_counter>=cfg.max_delay
                  fprintf(SerPor, '%c', '1');
                  signal=0;
              else
