function poll_for_data_preproc_GMM_Phantom(SubjectID, sessionN, cfg)
...
while 1 %length(files)
    ses_start=tic;
    time_diff
    pause_time=2-time_diff;
    waiting_time=0;
    GrabVol=tic;
    pause(pause_time); %CHANGE for MRI !!!!!!!!!!!!!!!!!!
     ...
     %   myvol2=datavol(maskvol2>0);
          %  myvol=(myvol-mean(myvol))/std(myvol);
            ...
            if SerPor.BytesAvailable
                ...
                vol_counter=1;
                if signal
                    tc_rt=tc(numProper); %((mean(myvol1)-mean(myvol2))/mean(myvol2))*100;
       ...
       % onset(end+1)=str2double(fscanf(SerPor, '%s'));
          ...
          if signal>0
              ...
              [MM(counter).Priors, MM(counter).Mu, MM(counter).Sigma, MM(counter).Pix, gmmcluster] = ...
                  EM_update_directMethod_upd(tc_rt(end), MM(counter-1).Priors, MM(counter-1).Mu, MM(counter-1).Sigma, MM(counter-1).Pix);
              ...
              if length(clustering)>1
                  if gmmcluster>clustering(end-1)
                      fprintf(SerPor, '%c', '1');
                      signal=0;
                  else
                      if vol_counter>=cfg.max_delay
                          signal=0;
                      else
                          vol_counter=vol_counter+1
                          %  break;
